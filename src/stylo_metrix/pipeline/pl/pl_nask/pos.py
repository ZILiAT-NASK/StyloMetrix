...
def verb_future(token):
    if 'fin' in token.tag_.split(":") and 'perf' in token.tag_.split(':'):
        return 'futs'  # zrobię
    if 'bedzie' in token.tag_.split(":") and token.dep_ not in 'aux'.split():
        return 'futs'  # będzie (zielony)
    if ('praet' in token.tag_.split(":") or 'inf' in token.tag_.split(":")) \
            and any('bedzie' in ch.tag_.split(':') for ch in token.children):
...
def verb_voice(token):
    if ('praet' in token.tag_ and any(token.text.endswith(suf) for suf in "bym byś by byśmy byście".split())) \
            or any(any(w == t.text for t in token.children) for w in "bym byś by byśmy byście".split()) \
            or  any(word in (t.text for t in token.children) for word in "gdyby jakby".split()):
        return 'cond'
    if ('impt' in token.tag_.split(":")) or \
            any(ch.dep_ == 'aux:imp' for ch in token.children):
        return 'impt'
