...
class L_NCONT_A(Metric):
    ...
    def count(doc):
        search = [
            token.text for token in doc._.words if token._.content_word == "noncont"
        ]
        result = incidence(doc, search)
        debug = {"TOKENS": search}
        return result, debug
...
class L_CONT_T(Metric):
    ...
    def count(doc):
        search = set(
            token.text for token in doc._.words if token._.content_word == "cont"
        )
        result = incidence(doc, search)
        debug = {"TOKENS": search}
        return result, debug
...
class L_CONT_L(Metric):
    ...
    def count(doc):
        search = set(
            token.lemma_ for token in doc._.words if token._.content_word == "cont"
        )
        result = incidence(doc, search)
        debug = {"TOKENS": search}
        return result, debug
